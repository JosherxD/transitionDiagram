...
class AutomataView:
    ...
    def solicitar_secuencia(self):
        """Abre ventana emergente para solicitar la secuencia"""
        def _crear_dialog():
            dialog = tk.Toplevel(self.root)
            dialog.title("Secuencia a Validar")
            ...
            try:
                ...
                pass  # Usar posición por defecto si falla
            
            return dialog
        
        dialog = ErrorHandler.safe_execute(
            _crear_dialog,
            ErrorHandler.handle_ui_error,
            "Creando ventana de secuencia"
        )
        
        if not dialog:
            return None
        
        return self._configurar_dialog_secuencia(dialog)
    
    def _configurar_dialog_secuencia(self, dialog):
        """Configura el contenido del diálogo de secuencia"""
        # Contenido de la ventana
        ttk.Label(dialog, text="Ingrese la secuencia a validar:", font=("Arial", 10, "bold")).pack(pady=10)
        
        secuencia_entry = ttk.Entry(dialog, width=30, font=("Arial", 10))
        secuencia_entry.pack(pady=10)
        secuencia_entry.focus()
        
        # Validación en tiempo real
        def validar_entrada(value):
            if not value:
                return True
            if hasattr(self.controller, 'simbolos'):
                simbolos_validos = set(self.controller.simbolos)
                for char in value:
                    if char not in simbolos_validos:
                        return False
            return True
        
        UIHelpers.setup_field_validation(dialog, secuencia_entry, validar_entrada)
        
        # Variable para almacenar el resultado
        self.secuencia_resultado = None
        
        def aceptar():
            secuencia = secuencia_entry.get().strip()
            if not secuencia:
                messagebox.showerror("Error", "Debe ingresar una secuencia")
                return
            ...
            for char in secuencia:
                if char not in simbolos_validos:
                    messagebox.showerror("Error", f"La secuencia contiene el símbolo '{char}' que no está en el alfabeto")
                    return
            
            self.secuencia_resultado = secuencia
            dialog.destroy()
        ...
        ttk.Button(button_frame, text="Aceptar", command=aceptar).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="Cancelar", command=cancelar).pack(side=tk.LEFT, padx=10)
        
        # Permitir Enter para aceptar
        secuencia_entry.bind('<Return>', lambda e: aceptar())
        
        # Esperar a que se cierre la ventana
        dialog.wait_window()
        
        return self.secuencia_resultado
